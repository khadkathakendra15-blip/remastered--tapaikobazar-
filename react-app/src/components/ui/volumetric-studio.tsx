...
type RoomProps = {
  ...
  lightColor?: string;
  spots?: number[];
  /** Per-spot beam colour ("r,g,b"); falls back to lightColor where omitted. */
  spotColors?: string[];
  vignette?: number;
...
function Room({
  ...
  const colorAt = (i: number) => (spotColors && spotColors[i]) || lightColor;
  const poly = useMemo(
    ...
    >
      ...
      >
        <div
          ...
          style={{
            clipPath: poly([tl, tr, br, bl]),
            background: spots.map((x, i) => `radial-gradient(ellipse 25% 40% at ${x}% 68%, rgba(${colorAt(i)},0.2) 0%, transparent 70%)`).join(", ")
          }}
        />
        <div
          className="absolute inset-0"
        ...
        <div
          ...
          style={{
            clipPath: poly([[0, 100], [100, 100], br, bl]),
            background: spots.map((x, i) => `radial-gradient(ellipse 35% 30% at ${x}% 80%, rgba(${colorAt(i)},0.08) 0%, transparent 60%)`).join(", ")
          }}
        />
      </div>
      <div
            ...
            <div className="relative mt-[6px] w-[54px] h-[64px] flex justify-center perspective-near">
              ...
              <div className="absolute bottom-[-6px] w-[58px] h-[18px] rounded-[50%] border-2 border-zinc-900 shadow-[0_10px_15px_rgba(0,0,0,1)] flex items-center justify-center z-10 overflow-hidden"
                 ...
                 <div className="w-[34px] h-[10px] rounded-[50%] transition-all duration-700"
                      style={{
                        background: lightsOn ? `rgb(${colorAt(i)})` : '#111',
                        boxShadow: lightsOn
                          ? `0 0 20px 8px rgba(${colorAt(i)},0.9), inset 0 0 8px rgba(${colorAt(i)},1)`
